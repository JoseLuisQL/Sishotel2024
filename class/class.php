...
<?php
	...
	";
		...
		if (limpiar(isset($_POST['fechaautorizacion'])) && limpiar($_POST['fechaautorizacion'] != "")) {
		...
		if (isset($_FILES['imagen2']['type'])) {
			$tipo_archivo = $_FILES['imagen2']['type'];
		} else {
			$tipo_archivo = '';
		}
		if (isset($_FILES['imagen2']['size'])) {
		...
		##################  SUBIR LOGO #3 ######################################
		//datos del arhivo  
		if (isset($_FILES['imagen3']['name'])) {
			$nombre_archivo = $_FILES['imagen3']['name'];
		} else {
			$nombre_archivo = '';
		}
		if (isset($_FILES['imagen3']['type'])) {
			$tipo_archivo = $_FILES['imagen3']['type'];
		} else {
			$tipo_archivo = '';
		}
		if (isset($_FILES['imagen3']['size'])) {
			$tamano_archivo = $_FILES['imagen3']['size'];
		} else {
			$tamano_archivo = '';
		}
		//compruebo si las características del archivo son las que deseo  
	...
	###################### FUNCION REGISTRAR PERIFL DE HOTEL #######################
	...
	################################ CLASE BANNER DE HOTEL ################################

	########################### FUNCION REGISTRAR BANNER DE HOTEL ###########################

	public function RegistrarBanner()
	{
		self::SetNames();
		if (empty($_POST["nombrebanner"]) || empty($_POST["descbanner"]) || empty($_FILES["imagenbanner"]["name"])) {
			echo "1";
			exit;
		}
		

		$nombrebanner = limpiar($_POST["nombrebanner"]);
		$descbanner = limpiar($_POST["descbanner"]);
		$imagenbanner = $this->uploadImage($_FILES["imagenbanner"]);

		$query = "INSERT INTO banner (nombrebanner, descbanner, imagenbanner) VALUES (?, ?, ?)";
		$stmt = $this->dbh->prepare($query);
		$stmt->bindParam(1, $nombrebanner);
		$stmt->bindParam(2, $descbanner);
		$stmt->bindParam(3, $imagenbanner);
		$stmt->execute();

		echo "<span class='fa fa-check-square-o'></span> El banner ha sido registrado exitosamente";
		exit;
	}
	...
	public function BannerPorId($codbanner)
	{
		self::SetNames();
		$sql = "SELECT * FROM banner WHERE codbanner = ?";
		$stmt = $this->dbh->prepare($sql);
		$stmt->execute(array($codbanner));
		$num = $stmt->rowCount();
		if ($num == 0) {
			return null;
		} else {
	...
	{
		self::SetNames();
		if (empty($_POST["codbanner"]) || empty($_POST["nombrebanner"]) || empty($_POST["descbanner"])) {
			echo "1";
			exit;
		}

		$codbanner = limpiar($_POST["codbanner"]);
		$nombrebanner = limpiar($_POST["nombrebanner"]);
		$descbanner = limpiar($_POST["descbanner"]);
		$imagenbanner = empty($_FILES["imagenbanner"]["name"]) ? $_POST["imagenbanner_actual"] : $this->uploadImage($_FILES["imagenbanner"]);

		$sql = "UPDATE banner SET nombrebanner = ?, descbanner = ?, imagenbanner = ? WHERE codbanner = ?";
		$stmt = $this->dbh->prepare($sql);
		$stmt->bindParam(1, $nombrebanner);
		$stmt->bindParam(2, $descbanner);
		$stmt->bindParam(3, $imagenbanner);
		$stmt->bindParam(4, $codbanner);
		$stmt->execute();

		echo "<span class='fa fa-check-square-o'></span> El banner ha sido actualizado exitosamente";
		exit;
	}
	...
	public function EliminarBanner()
	{
		...
		if (isset($_SESSION['acceso']) && $_SESSION['acceso'] == "administrador") {
			if (isset($_POST["codbanner"])) {
				$sql = "DELETE FROM banner WHERE codbanner = ?";
				$stmt = $this->dbh->prepare($sql);
				$codbanner = limpiar($_POST["codbanner"]);
				$stmt->bindParam(1, $codbanner);
	
				try {
					$stmt->execute();
					ob_end_clean(); // Limpiar buffer de salida
					echo json_encode(['status' => 'success', 'message' => 'El banner ha sido eliminado exitosamente.']);
				} catch (PDOException $e) {
					ob_end_clean(); // Limpiar buffer de salida en caso de error
					echo json_encode(['status' => 'error', 'message' => 'Error al eliminar el banner: ' . $e->getMessage()]);
				}
			} else {
				ob_end_clean(); // Limpiar buffer de salida
				echo json_encode(['status' => 'error', 'message' => 'Código de banner no proporcionado.']);
			}
		} else {
			ob_end_clean(); // Limpiar buffer de salida
			echo json_encode(['status' => 'error', 'message' => 'No tienes permiso para eliminar este banner.']);
		}
		exit;
	}
	...
	########################### FUNCION ELIMINAR BANNER DE HOTEL ############################

	############################ FIN DE CLASE BANNER DE HOTEL #############################
	...
	public function RegistrarNoticias()
	{
		...
		if ($num == 0) {
			...
			if (limpiar($_POST["statusnoticia"] == 'PUBLICADA')) {
				$publicado = limpiar(date("Y-m-d h:i:s"));
			} else {
				$publicado = limpiar("0000-00-00 00:00:00");
			}
...
public function ListarHabitacionesxTipos()
	...
	self::SetNames();
	$sql = "SELECT * FROM tiposhabitaciones 
	LEFT JOIN tarifas ON tiposhabitaciones.codtipo = tarifas.codtipo 
	LEFT JOIN habitaciones ON tarifas.codtarifa = habitaciones.codtarifa 
	GROUP BY tiposhabitaciones.codtipo ASC";
	foreach ($this->dbh->query($sql) as $row)
	{
		$this->p[] = $row;
	}
	...
	public function ListarHabitacionesxTipos()
	{
		...
		$sql = "SELECT * FROM tiposhabitaciones 
            LEFT JOIN tarifas ON tiposhabitaciones.codtipo = tarifas.codtipo 
            LEFT JOIN habitaciones ON tarifas.codtarifa = habitaciones.codtarifa 
            GROUP BY tiposhabitaciones.codtipo 
            ORDER BY tiposhabitaciones.codtipo ASC";
		foreach ($this->dbh->query($sql) as $row) {
			$this->p[] = $row;
		}
		return $this->p;
	...
	public function RegistrarClientes()
	{
		...
		if ($num == 0) {
			...
			if (limpiar(isset($_POST['limitecredito']))) {
				$limitecredito = limpiar($_POST["limitecredito"]);
			} else {
				$limitecredito = limpiar("0.00");
			}
			$fechaingreso = limpiar(date("Y-m-d"));
			$stmt->execute();
	...
	public function RegistrarInsumos()
	{
		...
		if ($num == 0) {
			...
			if (limpiar($_POST['fechaexpiracion'] == "")) {
				$fechaexpiracion = "0000-00-00";
			} else {
				$fechaexpiracion = limpiar(date("Y-m-d", strtotime($_POST['fechaexpiracion'])));
			}
			$codproveedor = limpiar($_POST['codproveedor']);
	...
	public function RegistrarProductos()
	{
		...
		if ($num == 0) {
			...
			if (limpiar($_POST['fechaelaboracion'] == "")) {
				$fechaelaboracion = "0000-00-00";
			} else {
				$fechaelaboracion = limpiar(date("Y-m-d", strtotime($_POST['fechaelaboracion'])));
			}
			if (limpiar($_POST['fechaexpiracion'] == "")) {
				$fechaexpiracion = "0000-00-00";
			} else {
	...
	public function RegistrarCompras()
	{
		...
		if (empty($_SESSION["CarritoCompra"])) {
			...
			if ($num == 0) {
				...
				if (limpiar($_POST["tipocompra"] == "CONTADO")) {
					$formacompra = limpiar($_POST["formacompra"]);
				} else {
					$formacompra = "CREDITO";
				}
				if (limpiar($_POST["tipocompra"] == "CREDITO")) {
					$fechavencecredito = limpiar(date("Y-m-d", strtotime($_POST['fechavencecredito'])));
				} else {
				...
				for ($i = 0, $iMax = count($detalle); $i < $iMax; $i++) {
					...
					if (limpiar($detalle[$i]['fechaelaboracion'] == "")) {
						$fechaelaboracionc = "0000-00-00";
					} else {
						$fechaelaboracionc = limpiar(date("Y-m-d", strtotime($detalle[$i]['fechaelaboracion'])));
					}
					if (limpiar($detalle[$i]['fechaexpiracion'] == "")) {
						$fechaexpiracionc = "0000-00-00";
					} else {
						$fechaexpiracionc = limpiar(date("Y-m-d", strtotime($detalle[$i]['fechaexpiracion'])));
					}
					$stmt->execute();
		...
		WHERE codproducto = '" . limpiar($detalle[$i]['txtCodigo']) . "'";
			   ...
			   ";
						...
						if (limpiar($detalle[$i]['fechaelaboracion'] == "")) {
							...
							$fechaelaboracion = limpiar(date("Y-m-d", strtotime($detalle[$i]['fechaelaboracion'])));
						}
						if (limpiar($detalle[$i]['fechaexpiracion'] == "")) {
							$fechaexpiracion = "0000-00-00";
						} else {
							$fechaexpiracion = limpiar(date("Y-m-d", strtotime($detalle[$i]['fechaexpiracion'])));
						}
						$codproveedor = limpiar($_POST['codproveedor']);
	...
	public function RegistrarArqueoCaja()
	{
		...
		if ($num == 0) {
			...
			if (limpiar(isset($_POST['ingresos']))) {
				$ingresos = limpiar($_POST['ingresos']);
			} else {
				$ingresos = limpiar('0.00');
			}
			if (limpiar(isset($_POST['egresos']))) {
				$egresos = limpiar($_POST['egresos']);
			} else {
				$egresos = limpiar('0.00');
			}
			if (limpiar(isset($_POST['creditos']))) {
				$creditos = limpiar($_POST['creditos']);
			} else {
				$creditos = limpiar('0.00');
			}
			if (limpiar(isset($_POST['abonos']))) {
				$abonos = limpiar($_POST['abonos']);
			} else {
				$abonos = limpiar('0.00');
			}
			if (limpiar(isset($_POST['dineroefectivo']))) {
				$dineroefectivo = limpiar($_POST['dineroefectivo']);
			} else {
				$dineroefectivo = limpiar('0.00');
			}
			if (limpiar(isset($_POST['diferencia']))) {
				$diferencia = limpiar($_POST['diferencia']);
			} else {
				$diferencia = limpiar('0.00');
			}
			if (limpiar(isset($_POST['comentarios']))) {
	...
	public function RegistrarReservaciones()
	{
		...
		if (isset($_FILES["sel_file"])) {
			...
			//datos del arhivo
			if (isset($_FILES['sel_file']['name'])) {
				$nombre_archivo = limpiar($_FILES['sel_file']['name']);
			} else {
				$nombre_archivo = '';
			}
			if (isset($_FILES['sel_file']['type'])) {
				$tipo_archivo = limpiar($_FILES['sel_file']['type']);
			} else {
				$tipo_archivo = '';
			}
			if (isset($_FILES['sel_file']['size'])) {
				$tamano_archivo = limpiar($_FILES['sel_file']['size']);
			} else {
				$tamano_archivo = '';
			}
